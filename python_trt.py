...
class Detector():
    def __init__(self, model_path, dll_path):
        ...
        self.yolov5.Detect.argtypes = [c_void_p, c_int, c_int, c_float, c_float, POINTER(
            c_ubyte), npct.ndpointer(dtype=np.float32, ndim=2, shape=(50, 6), flags="C_CONTIGUOUS")]
    ...
    def predict(self, img):
        ...
        self.yolov5.Detect(self.c_point, c_int(rows), c_int(cols), c_float(
            0.5), c_float(0.4), img.ctypes.data_as(POINTER(c_ubyte)), res_arr)
...
def visualize(img, bbox_array):
    for temp in bbox_array:
        ...
        img = cv2.rectangle(img, (int(x1), int(y1)),
                            (int(x2), int(y2)), (105, 237, 249), 1)
        img = cv2.putText(img, "class:"+str(clas)+" "+str(round(score, 2)), (int(
            temp[0]), int(temp[1])-5), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (105, 237, 249), 1)
    return img


det = Detector(model_path=b"/home/a100/Yolov5_Tensorrt_Win10/build_1/yolov5.engine",
               dll_path="/home/a100/Yolov5_Tensorrt_Win10/build/libyolov5.so")  # b'' is needed
