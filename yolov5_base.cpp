...
static const int INPUT_H = INPUT_H_replace;
static const int INPUT_W = INPUT_W_replace;
static const int CLASS_NUM = CLASS_NUM_replace;
...
extern "C" API void Detect(void *h, int rows, int cols, unsigned char *src_data, float (*res_array)[6], float conf_thres, float nms_thres)
{
    ...
    nms(res, &trt->prob[0 * OUTPUT_SIZE], conf_thres, nms_thres);
    int len = res.size();
